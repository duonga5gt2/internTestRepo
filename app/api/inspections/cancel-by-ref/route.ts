...
interface CancelInspectionByRefBody {
    contact: ContactLookupInput;
    property?: PropertyLookupInput;
    targetScheduledAt: string; // which appointment
    reason?: string;
}

export async function POST(req: NextRequest) {
    try {
        const body = (await req.json()) as CancelInspectionByRefBody;
        ...
        `,
            ...
            const cancelNotes =
                body.reason ?? 'Inspection cancelled via AI assistant.';
        ...
        console.error('Error cancelling inspection by ref', err);
        return NextResponse.json(
